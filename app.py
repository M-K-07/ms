...
mydb=mysql.connector.connect(host='localhost',user='root',password='1234',db='music')
Session(app)
@app.route('/')
def index():
...
def uploadmusic():
    if session.get('creater'):
        ...
        if request.method=="POST":
            ...
            flash(f'song {album_name} of {album_name} uploaded successfully!' )
            mydb.commit()
...
def uploadmusic():
    if session.get('creater'):
        ...
        if request.method=="POST":
            ...
            mydb.commit()
            cursor.close()
            flash(f'{song_name} added successfully!')
            return redirect(url_for('songsdashboard'))
